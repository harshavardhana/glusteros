...
do_manual_iface_config ()
{
    ...
    do
      local ERR;
      op=$( dialog --title "[ Interface Configuration ]" --ok-label Apply \
	  --extra-label Modify --cancel-label Back \
	  --inputmenu "settings of ${iface}" 0 40 10 \
	  "IP Address:" "${ip}" \
	  "Subnet Mask:" "${nm}" \
	  "Gateway:" "${gw}" \
          "Hostname:" "${hn}" 2>&1 >&3 );
...
do_manual ()
{
   local eth_ifaces;
	...
	eth_ifaces="${eth_ifaces} ${iface} '$(get_iface_mac ${iface})'";
      ...
      iface=$( sh -c "dialog --title '[ IP Configuration ]' --ok-label Select --cancel-label Done --menu 'Choose interface to configure IP' 0 0 0 ${eth_ifaces}" \
	  2>&1 >&3 ) || break;
      
      [ "${iface}" = "manual" ] && {
	  iface=$( dialog --ok-label Select --cancel-label Back \
	      --inputbox "Enter name of interface to configure ip:" \
	      0 0 eth0 2>&1 >&3 ) || continue;
      }
